...
(function($) {
    ...
    function uploadFile(file, args) {
        ...
        function fileloaded(presponse) {
            ...
            if(presponse != undefined && presponse.currentTarget!=undefined){
                ...
                if(presponse.currentTarget.responseText!=undefined){
                    if(String(presponse.currentTarget.responseText).indexOf('<div class="error">')==0){
                        ...
                        if (jQuery(args.cthis).children('.feedback').length>0) {
                            jQuery(args.cthis).children('.feedback').html(String(presponse.currentTarget.responseText));
                            setTimeout(function(){
                                jQuery(args.cthis).children('.feedback').html('');
                            },1500);
                        }
                        return false;
                    }
            ...
            if (args != undefined && args.cthis != undefined) {
                //console.log(args.cthis);
                    if (jQuery(args.cthis).next().hasClass('feedback')) {
                        jQuery(args.cthis).next().html('<div class="success">file uploaded</div>');
                        setTimeout(function(){
                            jQuery(args.cthis).next().html('');
                        },1500);
                    }
                if (jQuery(args.cthis).children('.feedback').length>0) {
                    jQuery(args.cthis).children('.feedback').html('<div class="success">file uploaded</div>');
                    setTimeout(function(){
                        jQuery(args.cthis).children('.feedback').html('');
                    },1500);
                }
            ...
            if (args != undefined && args.uploadlist != undefined) {
                for (i = 0; i < jQuery(args.uploadlist).children().length; i++) {
                    var $cache = jQuery(args.uploadlist).children().eq(i);
