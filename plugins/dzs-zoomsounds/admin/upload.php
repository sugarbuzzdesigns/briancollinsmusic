...
if(isset($_FILES['file_field']['tmp_name'])){
    ...
    if($sw==true) {  
        die('<div class="error">invalid extension - disallowed_filetypes (patched by x4151rch)</div><script>hideFeedbacksCall()</script>');  
    }
    ...
    //==== checking for disallowed file types
        ...
        if($sw==true) {  
            die('<div class="error">invalid extension - disallowed_filetypes (patched by x4151rch)</div>');  
        }
